...
В статье [Green et al., 2023 (BTR)](https://doi.org/10.1101/2023.10.13.562252) авторы показали, что их графовый подход (SR‑GNN на восстановленных DAG) превосходит линейные последовательности. Однако в **публично доступной реализации** (репозиторий на GitHub) представлена только линейная версия BTR (sequence/path). Поэтому в нашем сравнении используется именно эта линейная версия, а также отдельно реализованный нами SR‑GNN на графах.
